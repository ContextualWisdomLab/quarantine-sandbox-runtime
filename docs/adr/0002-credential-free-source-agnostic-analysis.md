...
ISO/IEC 27037:2012 remains the current International Standard for
...
confirmed current after the 2018 review). Those four activities belong to
the handler who first takes custody of the evidence. This leaf is not
that handler.

NIST SP 800-61 Revision 3, which superseded the withdrawn Revision 2 on
3 April 2025, places incident response inside organization-wide
cybersecurity risk management described by CSF 2.0 (Nelson et al., 2025;
National Institute of Standards and Technology, 2024). Analysis evidence
supports those outcomes. It does not replace the organization’s incident
program.
...
2. Source identity is not required for analysis. The host may attach
   bounded source context (channel, submitter class, filename the host
   already knows). The runtime does not fetch, crawl, or replay the
   source.
...
6. Incident handling outcomes—prepare, detect, respond, recover—remain
   with the host’s program (Nelson et al., 2025).
