...
| Effective container network binding | Root `ContainerInspection.HostConfig` does not deserialize `NetworkMode` or an equivalent attachment set; `verify_effective_isolation` can validate the intended `qsr-net-*` object without proving that the running container is actually attached only to it. Issue #22 and Draft #23 now carry three otherwise-positive hostile RED cases: a different `NetworkMode`, the expected mode with the expected attachment absent, and the expected attachment plus an unexpected `podman` network. | P0 false-positive found; RED staged on Draft #23 | Execute the RED on current ancestry. After reproduced failure, require positive exact attachment to the runtime-owned deny-by-default network and reject missing or additional attachments, then prove external egress denial on real rootless Podman. |
...
| Ownership/idempotency | Draft #6 implements caller-scoped lease ownership/idempotency but is stale behind current root. | Implemented on stale descendant | After root integration, adopt protected parent non-force and reacquire exact-head evidence. |
...
- A separately inspectable internal network does not prove that the running container is attached to it; exact positive attachment to the intended network and absence of additional attachments must be bound before egress isolation is Verified.
...
- Draft #23 is a test/traceability-only child of exact root `#1@24526eb55cf5db48ea07079b314f7d1b676eb48d`; its RED now requires wrong-mode, missing-positive-attachment, and unexpected-extra-attachment cases to fail closed before port/readiness. Production repair is forbidden until the preserved RED actually executes and fails for the intended network-binding cause.
...
2. Execute Draft #23's strengthened effective network-binding RED; after reproduced failure, add the smallest exact-attachment GREEN and real negative-egress E2E without weakening the dedicated LSM gate.
