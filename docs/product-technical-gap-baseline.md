...
The same command lane stages exact PR-source trees before a read-only `noexec,nosuid,nodev` bind. Issue #26 identifies a separate artifact-identity defect: Linux treats `/` as the pathname-component separator, but `collect_regular_files` currently rewrites a literal `\\` in a valid Linux filename to `/` before hashing and copying. `tests/pr_source_artifact_path_identity_red.rs` now requires a literal-backslash filename to preserve the exact path and tree digest. Production remains unchanged until that RED executes for the intended `DigestMismatch` cause; causal GREEN is the smallest lossless Linux-path repair plus collision coverage proving `a\\b` and `a/b` remain distinct identities.
...
7. Execute #14 issue #26's Linux pathname-identity RED; after the intended digest/path failure, remove the lossy separator rewrite and add explicit `a\\b` versus `a/b` collision coverage before treating exact-source staging as evidence-complete.
