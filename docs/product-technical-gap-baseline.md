...
| Real command-execution backend | `RootlessPodmanAdapter::run_command_at` (ADR-0008, `src/infrastructure/podman.rs`) creates a fresh `--network none`, read-only-rootfs, all-capabilities-dropped, non-root sandbox per call, verifies the same P0 controls as the service lease, fails closed (does not fall back to static-only evidence) when a fast-exiting command's live process can no longer be observed -- ADR-0008's original static-fallback design was reversed 2026-09-03 after `tests/podman_command_execution_live_attestation.rs` established that static configuration is not positive proof of effective process state; see that ADR's correction note -- and observes completion with `podman wait`/`podman logs`. All four P0 REDs this PR's own description named (AppArmor complain-mode, `podman logs` nonzero-exit rejection, cleanup-failure-hidden-behind-`container_start`, and the live-attestation fail-closed behavior) are fixed and green in a real Linux/rootless-Podman run (`08db9d8`), not just proven locally in the original sense -- but still not CI-verified. Proven against a real rootless Podman installation (locally, Podman 6.1.0; three genuine cross-Podman-version deserialization/comparison gaps were found and fixed this way -- see ADR-0008) via `tests/podman_command_execution_e2e.rs`, `#[ignore]`d pending `.github#1590` the same way the service lease's own real-isolation tests are. | Implemented on active PR, real-isolation proven locally, not yet CI-verified | Land `.github#1590`'s dedicated LSM-capable runner so `tests/podman_command_execution_e2e.rs` runs as CI evidence, the same blocker the service lease already carries; re-run against Podman 5.8.4 (CI's pin) to confirm the three version-specific fixes and the live-attestation fail-closed behavior hold there too. |
