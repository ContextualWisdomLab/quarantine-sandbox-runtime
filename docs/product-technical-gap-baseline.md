...
| Dedicated positive effective-LSM evidence | Root exact `5c6a44bb...` positive-LSM job `101907363137` is still queued on `[self-hosted, linux, cwl-hostile-workload, selinux]`. Hosted negative rootless/AppArmor is GREEN but is not positive confinement evidence. | Independent release/security lane blocked on dedicated runner | `.github#1590` remains canonical owner. Do not substitute hosted negative evidence or weaken the LSM gate. |
| Typed backend-spawn evidence and provider-neutral UL | Earlier hosted runs reproduced generic `rootless_probe` invocation failure in different integration-test binaries. Issue #71 / Draft #72 first executed a missing-executable RED, then preserved `std::io::ErrorKind` in `bounded_command` and mapped only `NotFound`, `PermissionDenied`, `WouldBlock`/`OutOfMemory`, and wildcard `Other` through the Podman ACL. #72 adopted current root without force via two-parent merge `37abd630ad585301621e5cd1d5d6981dd8a31126`. Integrated CI `34177417805` found and repaired one inherited constructor expectation plus rustfmt. Exact `8850158a...`, coverage `101910682603`, then executed the intended UL RED: typed missing-executable classification passed while public `BackendInvocationFailed` rendered `Podman invocation failed during runtime_inspection` instead of `backend invocation failed during runtime_inspection`. Minimum production `4f3da8b925170c40e0ff76234aaac1c031bd6413` changes only that public display/doc wording to backend-neutral text. | Two causal REDs; typed spawn candidate plus minimum UL repair; current exact-head GREEN pending | Re-run all exact-head gates after this ledger commit. No retry, extra mutex, or isolation weakening follows from failure classification. Keep timeout/output-limit/wait/capture/nonzero-exit distinct. |
...
| Spawn failure RCA | `BoundedCommandError::Spawn(io::ErrorKind)` now preserves the OS class in #72; Podman ACL bounds it to `BackendInvocationFailureKind::{NotFound, PermissionDenied, ResourceExhausted, Other}`. Missing output pipe remains Capture; timeout/output-limit/wait/nonzero exit remain distinct. | Minimum typed evidence candidate on current-root ancestry | Use future real recurrence to identify the concrete OS class. Classification is evidence, not retry policy. |
| Consumer-neutral invocation failure text | #72 exact `8850158a.../101910682603` causally proved provider leakage in `BackendInvocationFailed` while the typed missing-executable test passed. Minimum `4f3da8b...` changes only the public wording/doc comment. | Causal RED -> minimum GREEN candidate | Reacquire exact-head tests/rustdoc/coverage after this documentation update; do not rename variants or broaden scope. |
...
1. Re-run current #72 after this ledger update. Require its causal provider-neutral RED to become exact-head GREEN without regressing typed spawn classification, rustfmt, Clippy, rustdoc, source/function/line/branch coverage, or hosted negative semantics.
2. Keep #72 Draft while dedicated positive-LSM, current review/thread requirements, and central security gates remain incomplete. If its exact hosted gates are GREEN, normally integrate only when protection allows; never force-restack.
...
4. Non-force restack issue #86 / Draft #92 and root children #89/#85/#88/#91 independently after the root/#72 chain stabilizes, preserving every child-owned delta and reacquiring exact-head evidence.
