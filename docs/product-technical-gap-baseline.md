...
| Root owned-production coverage and process-fixture stability | Exact root `8e51c28eea6f31a19f13811e926574f5d44ae757`, native CI `34166007416`, executed the focused empty-registry-authority and SELinux `unconfined` coverage cases. Hosted negative rootless/AppArmor `101877056046` was GREEN. Branch-coverage `101877056305` generated and uploaded artifact `10034315670` (`sha256:a4f3bd0be92769f31d7bcaa9eb191b8f15bac995e3af5de73de7e6d1204b8c81`) and failed only final complete-branch enforcement with production functions `189/189`, lines `1974/1974`, regions `2636/2637`, branches `456/460`. The four remaining Podman branch misses map exactly to downstream empty-token predicates at the SELinux runtime label, AppArmor normalized label/profile, and capability parser. `parse_process_security_top` uses `split_whitespace()`, requires at least eight PID-1 fields, and therefore cannot construct those empty tokens. Production commit `bd23862df508dfec8f24fe7dbc74b57d23491bac` removes only those parser-dominated predicates while retaining reachable `unconfined`, AppArmor enforce-mode, inspect-label, and nonzero capability checks. One `bounded_command.rs` region remains without an attributable zero-count source line/segment. | Focused tests executed; parser-dominated production simplification is the current candidate, not yet exact-head GREEN | Re-run exact current head and require the Podman branches to close without semantic regressions. Use fresh LLVM evidence to diagnose the single `bounded_command` region; do not guess, exclude it, shrink tests, or lower thresholds. |
...
| Central CodeQL terminal verdict | Exact root `8e51c28...` CodeQL `34166007420` detected languages and dispatched the current-head scan, but actions `101877954036` and python `101877954119` failed `Release runner or enforce current-head CodeQL verdict`; dispatch `101878903619` itself succeeded. | Current exact central required-workflow failure | `.github#1929` remains canonical owner. Do not reinterpret dispatch success as terminal CodeQL acceptance or patch leaf status. |
| Dependency Review availability | Exact root Security Scan `34166007426` dependency-review `101877918683` verified exact checkout/head and then failed `Check dependency review support`; actual Dependency Review was skipped while Trivy `101877918637`, OSV `101877918639`, and Scorecard `101877918665` succeeded independently. | Current exact central required-workflow failure | `.github#810` owns availability. Do not substitute other scanners for Dependency Review. |
| SAST | Exact root `8e51c28...` SAST Semgrep `34166007428` completed SUCCESS. | Current exact security evidence for that predecessor SHA only after source movement | Reacquire on every moved exact root and do not treat one security gate as full release authority. |
| Native runner acquisition | Exact `8e51c28...` acquired hosted Ubuntu runners, while positive-LSM `101877056232` remained queued with `runner_id=0` on `[self-hosted, linux, cwl-hostile-workload, selinux]`. | Hosted capacity available; dedicated positive confinement evidence still blocked | `.github#1590` owns dedicated positive-LSM runner capacity/evidence. Continue safe independent work and never substitute hosted negative evidence. |
...
1. Execute exact-head CI after `bd23862...`. Confirm the four parser-dominated Podman branches disappear while reachable `unconfined`, AppArmor enforce-mode, inspect-label, and nonzero capability checks remain behaviorally intact.
2. Use fresh exact LLVM JSON to identify the single remaining `bounded_command.rs` region. Repair only a real reachable gap or simplify proven instrumentation/dead structure; no exclusions, sample reduction, impossible synthetic input, or threshold changes.
