...
- #51/#55/#59 remain independent result/provenance/subject-binding prerequisites. #67 is now exact `1ca135d3ddd4e17fb59fd48ee3bff128c5f6fb50`: it retains the earlier executed self-verifiability RED and dead-serializer cleanup, and adds a test-only descriptor-identity RED requiring identical artifact bytes/request/profile with distinct admitted original leaf names to produce distinct `analysis_job_id` and job-bound evidence IDs. Production/schema behavior is unchanged; CI `35395166943` is queued.
...
| Analysis job identity can alias materially distinct canonical artifact descriptors when bytes/SHA and other current derivation inputs are identical. | #66/#67 current `1ca135d...`, CI `35395166943`; descriptor-identity RED is test-only. | Execute the current descriptor witness for the intended aliasing cause; then repair only after #54/#55 stable analyzer provenance can participate in a versioned independently verifiable derivation. |
...
8. Let #51/#55/#59 and #67 exact `1ca135d...` / CI `35395166943` execute independently without wake commits; #67 must first prove descriptor-identity aliasing before any new GREEN.
