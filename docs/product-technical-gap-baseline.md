...
Last reviewed on 2026-09-04 KST against dependency-root PR #1 source head `5dcabdbd5a61f9e42e474896e07ed9480dafc491`, root exact head `24526eb55cf5db48ea07079b314f7d1b676eb48d`, and protected `develop@60a85c7633e03b425b67159ec6822c8178cf87ea`. Draft #19 has now adopted that live root non-force while preserving its resource-attestation RED and standards traceability. This ledger distinguishes protected truth, active-PR implementation, checked-in RED evidence, backend-applied configuration evidence, live effective-runtime proof, and queued/cancelled checks. Predecessor evidence never transfers to a moved head.
...
| tmpfs / wall time | Launch applies bounded `/tmp` and `--timeout`, but current root does not deserialize/bind `HostConfig.Tmpfs` or `Config.Timeout`; inspect state alone would still not prove live enforcement. Draft #19 preserves REDs for missing/wrong hardening, contradictory/duplicate tmpfs options, widened writable mounts, timeout mismatch, cleanup/non-publication, and exact inspect state without live proof. | P0 RED preserved on root-current ancestry; fresh execution pending | Execute the reconciled RED. After reproduced failure, add only the smallest inspect-binding GREEN, then require live cgroup/mount/wall-time proof. |
...
| Ownership/idempotency | Draft #6 implements caller-scoped lease ownership/idempotency but remains gated behind root verification. | Implemented on descendant | Preserve caller-scoped correlation separately from backend invocation/resource ownership. |
...
- Root source head `5dcabdbd...` removes the stale Podman-4.9 fixture expectation and carries rustfmt-aligned infrastructure source; root exact head `24526eb...` updates only this ledger after that source point.
- Draft #19 predecessor exact `b81a21be230baba03a8ea114773cc7805753c11d` finally acquired GitHub-hosted Ubuntu 24.04 runners in CI `33866886182`. Exact checkout succeeded, but the intended resource RED did not execute: `verify=101003665623` stopped at `cargo fmt --check` on stale pre-root source formatting, while `coverage=101003665247` stopped earlier in `tests/application_service_contracts.rs` at the stale `--no-hostname` expectation. `podman-e2e=101003665602` reached real isolation acceptance but failed independently; leak rejection completed successfully. These are ancestry/precondition failures, not resource-attestation RED evidence.
- Draft #19 therefore adopted root `24526eb...` non-force instead of applying a production resource fix to stale ancestry. The preserved resource RED must now execute on the reconciled exact head and fail for `resource_limits` before any production GREEN.
- Generic hosted-runner availability is no longer a static outage: #19's predecessor run was assigned hosted runners after a long queue. New current-head jobs may still queue and remain incomplete evidence; central queue-health ownership stays `.github#712`.
...
- Current descendants are not destructively rewritten. #6/#9/#10/#13/#14 retain valid implementation/test deltas; #18 retains a RED-only delta on an older command-parent snapshot; #19 has now adopted current root while preserving its RED/traceability history.
...
1. Execute the dependency root exact-head CI/security runs; repair only an actually reproduced current-head failure and merge normally only after approval/rules/effective-runtime gates are satisfied.
2. Execute Draft #19's now-reconciled resource-attestation RED. Only after the intended `resource_limits` failure is observed may the smallest backend-applied tmpfs/timeout binding GREEN be added; live cgroup/mount/wall-time proof remains a separate required increment.
3. Execute Draft #23's effective network-binding RED; after reproduced failure, require positive exact attachment to the runtime-owned deny-by-default network and real negative-egress proof.
4. Reconcile #6/#9/#10/#13/#14 dependency-first after root integration; remove or independently prove the shared `Command::spawn` `WouldBlock` retry before protected integration, and execute preserved command identity/Podman-4.9 regressions on current ancestry.
