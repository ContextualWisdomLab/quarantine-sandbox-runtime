...
| Subprocess spawn pressure | Repeated old exact #9/#10 native-CI specimens collapsed backend spawn faults into `BackendInvocationFailed { operation: "backend_security_info" }`. Current `BoundedCommandRunner` maps every `Command::spawn()` error to unclassified `Spawn`, and missing stdout/stderr capture also uses `Spawn`, so `NotFound`, `PermissionDenied`, resource pressure and pipe-capture failures cannot be distinguished causally. Issue #71 / Draft #72 adds a deterministic missing-executable RED; review hardened the fixture to a test-owned empty directory so the absent child path cannot be replaced by ambient `/tmp` state. | Focused observability RED checked in; production unchanged pending causal execution | Execute #72 on its unchanged exact head. After the intended missing API/variant RED, preserve a bounded stable spawn class (`not_found`, `permission_denied`, resource-exhausted/would-block, `other`) with a wildcard for future Rust `ErrorKind` variants, keep capture/wait/timeout/output/nonzero-exit distinct, and do not add generic retry. |
...
- Root PR #1 exact `9bae89ca75575055f928c24546dd7c51e4f2049b` is a test-only repair of stale fake-Podman security fixtures discovered by executed predecessor CI; production Rust/schema are unchanged and fresh exact-head native/security evidence remains required.
- Issue #24's checked-in RED executed on exact root head `0f765af1a4eea83029febee3b24c55cd7e7ce4e1`. CI run `33915538506` checked out that exact SHA. `verify=101191626830` failed only when `ci_runs_on_every_integrated_protected_develop_head` observed stale `push.branches: [main]`; formatter, repository policy and preceding tests passed. `coverage=101191626974` and `branch-coverage=101191627084` independently reached the same causal assertion. The hosted negative rootless/AppArmor lane `101191627075` completed successfully; the dedicated positive-LSM lane remains a separate prerequisite.
- The smallest causal issue #24 repair is commit `034155c804baffd7b57a97da84a6037d79dd6a96`: native CI `push.branches` targets `develop` and changes no other workflow semantics. `pull_request` remains enabled, no `paths`/`paths-ignore` filters were added, current-head cancellation remains scoped by PR/run identity, hosted lanes remain `ubuntu-24.04`, and the positive-LSM lane remains `[self-hosted, linux, cwl-hostile-workload, selinux]`.
...
- Issue #71 is now a focused prerequisite for backend-spawn RCA. Draft #72 is intentionally RED-only until the deterministic missing-executable contract executes; transient queued job IDs stay in PR/Issue/central owner-path metadata rather than this versioned ledger.
...
1. Reacquire exact-head CI/security/coverage on PR #1. If its refreshed fake-Podman fixtures reveal another fixture-contract defect, repair the test fixture rather than weakening effective-security validation.
2. Execute Draft #72's deterministic missing-executable RED. Only after the intended missing `BackendInvocationFailureKind` / `BackendSpawnFailed` contract is observed may the smallest classification GREEN be added; keep capture/wait/timeout/output/nonzero-exit distinct and add no generic retry.
3. Once #1 has qualifying review and all exact-head gates, merge normally. Then prove the native CI run materializes on the exact protected `develop` integration SHA; only that satisfies issue #24's post-integration completion gate.
