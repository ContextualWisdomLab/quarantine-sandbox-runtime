...
| Repository workflow SHA-pin validation | Issue #86 / Draft #92 exact `54299dd50748cb5237530299ed6e1fa6e3845781`, native CI `34150580702`, verify `101831850483` executed the original hardened RED: exact checkout/dependency lock/current validator passed, then focused policy tests failed. Minimum policy commit `eeec132cc61fbc86a07c09f75be828afc4d566c3` repairs multi-file discovery, `- uses:` parsing, and missing/unreadable/empty workflow admission, but review `5134607273` found its blanket `@<sha>` rule falsely rejects GitHub's same-repository, same-running-commit `$/...` action/reusable-workflow syntax. Test-only `c4173ec8211debaaa753de3b027e677c2b73dbf4` adds `$/.github/actions/...` and `$/.github/workflows/...` positive controls; doctoring `d30dc89f2d72941d1a7a8bae0d772de98a306cdf` records the boundary. | Original causal RED + incomplete minimum candidate + new checked-in RED | Execute the `$/` self-reference RED causally before any further production change. After that, admit only commit-bound `$/` references before applying the existing exact 40-character lowercase SHA rule to external dependencies; add no tag/branch exception. |
...
2. Execute issue #86 / Draft #92's checked-in same-repository `$/` self-reference RED above the first minimum policy repair. Only after that exact RED fails for the over-broad SHA rule may production admit the bounded commit-bound `$/` form; keep external references full-SHA-only and reacquire the full exact-head gate.
