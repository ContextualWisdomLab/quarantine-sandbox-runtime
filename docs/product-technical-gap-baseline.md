...
| Repository workflow SHA-pin validation | Issue #86 / Draft #92 exact `54299dd50748cb5237530299ed6e1fa6e3845781`, native CI `34150580702`, verify `101831850483` executed the hardened RED: exact checkout/dependency lock/current validator passed, then focused policy tests failed. The tested defects are the root validator's one-file scan, its failure to recognize admitted `- uses:` sequence-step syntax, and uncaught missing-workflow admission. Minimum policy commit `eeec132cc61fbc86a07c09f75be828afc4d566c3` now enumerates direct `.yml`/`.yaml` files deterministically, converts missing/unreadable/empty workflow surfaces to policy errors, recognizes both `- uses:` and `uses:`, and preserves the exact 40-character lowercase SHA rule. | Causal RED + minimum candidate; local five-case unit suite GREEN, exact moved-head CI pending | Reacquire the full current-head gate without weakening immutable-action policy. Do not treat predecessor `54299dd...` failure or local tests as GREEN for the moved candidate. |
...
2. Reacquire issue #86 / Draft #92 exact-head CI after minimum policy commit `eeec132...` plus current doctoring/gap ledger. If the focused five-case suite and unchanged repository validator gate are GREEN, continue through formatting/tests/clippy/rustdoc and applicable coverage/security jobs; repair only new causal failures.
