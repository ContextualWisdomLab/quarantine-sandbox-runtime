...
| tmpfs and wall-time bounds | PR #1 launch argv carries bounded `--tmpfs` and `--timeout` but does not deserialize `HostConfig.Tmpfs` or `Config.Timeout`. PR #19 RED rejects missing/wrong tmpfs configuration, missing `noexec`/`nosuid`/`nodev`, contradictory execution flags, conflicting size tokens, unexpected extra writable tmpfs mounts, timeout `0`, wrong positive timeout, and—critically—exact inspect configuration when no live runtime proof is supplied. Every failure requires stop/container/network cleanup and forbids port publication. | P0 RED on Draft PR #19 | Execute RED `f428746eade529b8a09a7fc79a67c17a11ddab7c`. First GREEN must add strict request-bound inspect parsing without treating it as effective proof. A second positive fixture must supply authoritative live cgroup/mount/time evidence before any lease can succeed. |
...
- PR #1 has no qualifying approval. Rules/review/security gates remain authoritative; administrator capability is not evidence.
...
- Draft PR #19 is a descendant RED, not release authority. Latest test-bearing commit is `f428746eade529b8a09a7fc79a67c17a11ddab7c`. The previous exact-head queue evidence belongs to its predecessor and does not transfer; the current head must materialize and execute its own focused/full lanes before production repair.
- Organization ruleset `18156473` (`CWL Central required workflows`) is active. GitHub Releases was empty at the last fresh release sweep; no immutable runtime release may be represented to Wardnet/contextual-orchestrator/Noema until re-read and actually published from an integrated protected candidate.
...
- Podman documents `--tmpfs` as a tmpfs mount and supports Linux mount options; the runtime requires `rw,noexec,nosuid,nodev` plus an exact size bound and no unrequested writable tmpfs surface. Inspect parsing must be order-insensitive and fail closed on missing, duplicate, contradictory or widened values.
...
2. Execute PR #19 RED `f428746eade529b8a09a7fc79a67c17a11ddab7c`. The intended current failures include acceptance of missing/contradictory/widened tmpfs evidence and the deeper case where exact inspect resource configuration can still reach lease/publication without live resource-enforcement proof.
