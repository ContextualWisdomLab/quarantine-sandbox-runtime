...
| Post-create container ownership | `launch_at` parses the long container ID returned by successful `podman create`, but start/inspect/top/port/failure cleanup and later `terminate_at` still address the generated `qsr-app-*` name. Issue #40 and test-bearing Draft #21 commit `8319e0be20a8bbeaeaec97a1d9685cd80856780a` model immediate name rebinding to a foreign container while the acquired ID remains valid. | P0 RED checked in; production unchanged | Execute #40 RED, then retain the acquired long ID as backend lifecycle/destructive authority while keeping the generated name as current correlation/public `sandbox_id` semantics unless versioned deliberately. |
...
5. Execute Draft #21's runtime-identity collision RED and issue #40's post-create lifecycle-ownership RED. Then introduce one testable collision-resistant invocation identity below consumer correlation/idempotency semantics and retain the acquired Podman long ID separately as backend lifecycle/destructive authority.
...
7. Reconcile #6/#9/#10/#13/#14 dependency-first without force. Overlapping foundation/CI deltas must be adopted rather than copied or reintroduced; then reconcile #18 on its current command parent.
