...
Last reviewed on 2026-09-07 KST against dependency-root Draft PR #1 exact `17e79aab32aa75efd26bcccf92415474af9bc69b`, the proven checkout-credential GREEN on predecessor `7482108c0b74f58f447722a98330f9ad44215eec`, the active production-coverage repair #83, and protected/default `develop@60a85c7633e03b425b67159ec6822c8178cf87ea`. This ledger distinguishes protected truth, active-PR implementation, checked-in RED evidence, backend-applied configuration evidence, live effective-runtime proof, queued/cancelled checks, and post-integration protected-head evidence. Predecessor evidence never transfers to a moved head.
...
| Rootless backend | Exact predecessor `7482108c0b74f58f447722a98330f9ad44215eec` executed on GitHub-hosted Ubuntu 24.04 and kept the hosted negative rootless/AppArmor lane GREEN. Positive per-sandbox LSM evidence is still a dedicated runner concern. | Real hosted negative-path evidence obtained | Re-prove every moved exact head; rootless capability does not prove per-sandbox LSM confinement. |
| Read-only / writable surfaces | Launch uses read-only rootfs, `--read-only-tmpfs=false`, bounded `/tmp` with `rw,noexec,nosuid,nodev`, image volumes ignored, no generated hosts file, no proxy inheritance, and no systemd/sdnotify integration. Unsupported Podman 4.9.3 `--no-hostname` remains absent. | Configured intent plus partial runtime inspection | Bind exact applied tmpfs mount set/options/size, then require live mount proof. |
| Privilege / namespace isolation | Current repair checks empty container Effective/Bounding caps and process Effective/Bounding/Inheritable/Permitted/Ambient sets, no-new-privileges, seccomp, user/PID/IPC namespaces, numeric non-root identity, and enforcing SELinux/AppArmor evidence. Ordinary hosted execution remains an explicit negative-LSM lane. | Negative effective-LSM evidence obtained; positive proof pending | Positive release evidence remains on the dedicated SELinux-capable runner; never reinterpret unavailable/unconfined evidence as Verified. |
| Missing Podman security fields | `ContainerInspection.EffectiveCaps`, `ContainerInspection.BoundingCaps`, and `NetworkInspection.dns_enabled` currently use `#[serde(default)]`, making missing fields indistinguishable from explicit empty/false secure values. Issue #73 / Draft #89 carries a focused test-only RED plus TRACEABILITY; production is unchanged. | P0 RED staged; causal execution pending | Execute #89 on its exact head. Only after the intended RED, remove those three secure defaults so absence becomes `MalformedIsolationInspection` while explicit `[]` / `false` remains valid observed evidence. |
...
| Credential isolation | P0 request has no provider/user credentials, arbitrary environment map, host device/runtime socket, or broad host mount. Root checkout actions now also use `persist-credentials: false`; predecessor `7482108...` verify proved that focused CI contract GREEN. | Active runtime contract; CI credential repair proven | Preserve on all descendants and reacquire exact-head evidence after movement. |
...
| Subprocess spawn pressure | Issue #71 / Draft #72 now owns bounded spawn-failure classification and provider-neutral public error text. The original missing-executable classification RED executed causally; current text RED still awaits exact-head execution. | Causal classification RED obtained; focused follow-up pending | Keep raw errno/provider diagnostics inside infrastructure ACL; do not add generic retry authority. |
...
- A missing required inspection member is unavailable evidence, not a secure default. Explicit empty/false values and absent values must remain distinguishable at the infrastructure ACL.
...
| Analyzer worker execution port | Issue #69 / Draft #70 crossed its Core-boundary compile RED and now carries Core-owned worker isolation/resource/lifecycle values plus a seven-required-control RED. Dependency-safe #78/#80/#82 separately own evidence-kind authority, exit/outcome binding, and exact cleanup identity. | Parent repair plus focused RED children staged | Execute current #70 exact RED first, then minimum causal GREEN and ordinary non-force child restacks. |
...
- The checkout-credential RED executed causally on predecessor `ffc16e1aca2bd2574ddd04025eb4bffe10406dee`; minimum root repair `7482108c0b74f58f447722a98330f9ad44215eec` changed only the five pinned checkout invocations to `persist-credentials: false`. Native CI `34089522598` verify `101640016166` completed SUCCESS and the focused credential contract passed; hosted negative rootless/AppArmor `101640015989` also passed.
- That same `7482108...` run exposed the next executable root gate: coverage and branch coverage failed after tests passed, with production totals lines `1927/1978`, functions `186/189`, regions `2567/2644`, branches `412/468`. Issue #83 owns the repair without threshold lowering or production exclusions.
- Current root `17e79aab32aa75efd26bcccf92415474af9bc69b` adds only deterministic Podman failure-matrix coverage over the proven credential repair. Native CI `34123216135` is current for that root and was still queued at the latest fresh read; no current coverage GREEN is claimed.
- Current root SAST `34123216154` completed successfully. Current Security Scan `34123216126` failed specifically because Dependency Review support was unavailable after exact checkout/head verification; independent OSV/Scorecard/other successful security jobs are not substitutes. `.github#810` owns that central support path. Current CodeQL `34123216169` remains separately required.
- Positive effective-LSM evidence is not inherited from hosted execution. The dedicated `[self-hosted, linux, cwl-hostile-workload, selinux]` lane remains an independent release gate tracked through `.github#1590`.
- Issue #24's protected-`develop` CI-trigger RED executed previously and its `push.branches: [develop]` repair remains on the root lineage. After normal integration, a native push-triggered CI run must still materialize on the exact protected `develop` integration SHA.
- Main command/release descendants #6 -> #9 -> #10 -> #13 -> #14 and artifact-analysis #18 must adopt root/parent movement only through non-force ancestry repair. Direct focused siblings/children retain valid deltas but predecessor checks never transfer.
...
The first release remains blocked until one exact integrated protected candidate carries complete owned statement/function/region/branch coverage and public rustdoc, realistic rootless isolation E2E, positive effective LSM/seccomp/capability/resource/network/cleanup evidence, required review/security/SAST gates, package smoke, SPDX SBOM, provenance, checksum/signature as supported, reproducibility, upgrade/rollback evidence, and an immutable artifact identity.

## Next bounded slices

1. Execute Draft #89's missing-Podman-isolation-field RED on the exact child head. After the intended failures prove current secure-default behavior, remove only the three `#[serde(default)]` attributes and reacquire exact-head GREEN; explicit empty capability arrays and `dns_enabled=false` remain valid observations.
2. Reacquire exact-head CI/security/coverage on root PR #1 issue #83 coverage candidate. Do not regress the proven checkout credential repair, protected-`develop` trigger, or positive-LSM separation.
3. Once #1 has qualifying review and all exact-head gates, merge normally. Then prove the native CI run materializes on the exact protected `develop` integration SHA.
...
6. Execute Draft #21's #20/#40 runtime identity and lifecycle-ownership REDs; then introduce collision-resistant invocation identity and exact acquired-ID lifecycle authority below consumer correlation/idempotency semantics.
...
8. Execute current #70 worker isolation-completeness RED, then minimum causal GREEN and non-force restack #78/#80/#82 without copying child-owned semantics into the parent.
9. Reconcile #6/#9/#10/#13/#14 dependency-first without force. Overlapping foundation/CI deltas must be adopted rather than copied or reintroduced; then reconcile #18 on its current command parent.
10. Publish the first immutable runtime release only from one exact integrated protected head, then hand off released version/digest pinning to consumer owner paths.
