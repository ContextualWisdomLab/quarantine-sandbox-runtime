...
| Repository workflow SHA-pin validation | `scripts/validate_repository.py` on current root has two false-negative paths: it reads only `.github/workflows/ci.yml`, and its `line.strip().startswith("uses:")` filter does not recognize the repository's normal YAML sequence-step form `- uses: ...`. Additional `.yml`/`.yaml` workflows are outside the scan, current `ci.yml` action steps are not actually inspected, and missing `ci.yml` raises rather than producing a deterministic policy result. Issue #86 / Draft #92 adds an independent primary-`ci.yml` list-item RED plus second `.yml`, `.yaml`, missing-workflow, and all-pinned controls. | Hardened RED-only child; production/policy implementation unchanged | Execute #92 hardened RED, then deterministically enumerate all direct workflow `.yml`/`.yaml` files, recognize admitted `- uses:` step syntax, and retain the exact 40-character lower-case SHA rule without exceptions. Discovery-only repair that preserves the current list-item false negative is invalid. |
...
2. In parallel, execute issue #86 / Draft #92 hardened repository-workflow action-discovery RED. After the primary `- uses:` parsing, multi-file discovery, and missing-workflow failures execute for their intended causes, make only the deterministic admitted-step/all-workflow SHA-pin admission repair and reacquire the moved exact head.
