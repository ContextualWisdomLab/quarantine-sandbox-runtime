...
Those tests shared a test-infrastructure pattern that is unsafe under concurrent Unix process creation: they created fake executable scripts at runtime with `fs::write`, and the ownership retry case rewrote the same executable path between phases. Rust upstream documents a Unix `Command::spawn` race in which spawning a newly written executable can fail with `ETXTBSY`; Rust's public `Command::spawn` contract also explicitly permits process-creation failures before a child is successfully started. The exact hosted jobs expose only the intentionally bounded public class `Other`, so this record does **not** claim that their raw OS error was observed to be `ETXTBSY`. It records that the repository fixture architecture matched a known upstream hazard and removes that hazard structurally rather than guessing the hidden errno.

Initial test-only structural repair:
...
- `f9f3d87509ea9c9ee633fb2f76b2b9b4091b7442` converts application-service ownership fixtures to the same model. The failed-launch retry changes only data configuration, never the executable inode.

The next exact lineage showed that the first conversion was too narrow rather than that production semantics were wrong. Exact `e94e6043bb7df0d557ed7d5f3d142ee62baed1a4`, CI `34337359199`, executed both immutable namespace regressions successfully and preserved acquired-ID cleanup authority. Verify stopped earlier at rustfmt on a stale multiline helper; formatter-only `6c5e16eadad62a0d56506e4e02186496c6805486` applies the exact formatter result. Branch coverage `102419783425` and coverage `102419783651` then reached `podman_command_execution_post_create_ownership_red::successful_create_binds_every_lifecycle_operation_to_the_acquired_container_id` after adjacent command-runtime suites had passed and ended in `CleanupFailed`; that regression still created its fake executable at runtime.

Test-only `fc94073a95e359d57537225b43ae28119e340e52` adds a dedicated immutable `command_owned_lifecycle` mode, and `a9c0aa0c01b56f09c3f77dd541ca58d8a72945a8` moves the post-create ownership regression to a unique symlink plus data-only configuration. Exact `a9c0aa0...`, native CI `34338451401`, then exposed the remaining shared helper in `tests/podman_command_execution.rs`: verify `102423292005` failed `run_command_at_cleans_up_when_container_start_fails` before the intended start-failure contract with `BackendSpawnFailed { operation: "backend_security_info", failure_kind: Other }`; production coverage `102423291940` independently failed `run_command_at_cleans_up_when_container_create_returns_a_malformed_identifier` with the same bounded Spawn/Other class while 14 neighboring command tests passed. Hosted negative rootless/AppArmor `102423291612` succeeded. These rotating precondition failures again identify the runtime-created executable helper as the remaining test-infrastructure defect; they are not authority to weaken start, malformed-create, cleanup, or acquired-ID semantics.

The final structural conversion is intentionally helper-level rather than a growing list of leaf exceptions. `b7496f7206ce13bedf71987cb4fd1aeb23041142` adds `source_script` to the checked-in immutable fake Podman: scenario bodies are sourced as data through `${0}.config` instead of being spawned as newly written executable inodes. `2ff079ef47d0c5fd38ac90bf2cf81d4aa203b1e6` changes `tests/podman_command_execution.rs::write_executable` to create only a unique symlink to the checked-in fixture and data-only `.script`/`.config` sidecars. The existing 15 command-runtime scenarios and assertions are otherwise retained, including backend-loss cases where removing `$0` now removes the invocation symlink so the following spawn and cleanup still fail closed as intended. Neither commit changes production Rust.

Native CI for exact `2ff079ef47d0c5fd38ac90bf2cf81d4aa203b1e6` is `34339464439`. At the time this record moved, verify `102426627936`, coverage `102426628422`, branch coverage `102426628350`, hosted negative `102426628311`, and dedicated positive-LSM `102426628555` were all newly materialized and had not yet produced transferable GREEN. The positive lane remains restricted to `[self-hosted, linux, cwl-hostile-workload, selinux]`.

No retry, sleep, mutex, lowered assertion, production-error remapping, or broadened public error surface was introduced. Fixture changes are test infrastructure only.
...
Primary files and APIs: `src/infrastructure/bounded_command.rs`, `src/infrastructure/podman.rs`, `src/application_service/mod.rs`, `src/lib.rs`, `tests/backend_spawn_failure_classification_red.rs`, `tests/fixtures/fake_podman.sh`, `tests/podman_command_execution.rs`, `tests/podman_command_execution_cleanup_red.rs`, `tests/podman_command_execution_namespace_config_red.rs`, `tests/podman_command_execution_post_create_ownership_red.rs`, and `tests/application_service_ownership.rs`. The final exact branch head must newly pass repository policy, rustfmt, full Rust tests, Clippy, rustdoc, complete owned-production statement/function/region/branch coverage, hosted negative confinement, dedicated positive effective-LSM, qualifying review, and central security/dependency gates. Predecessor GREEN does not transfer.
