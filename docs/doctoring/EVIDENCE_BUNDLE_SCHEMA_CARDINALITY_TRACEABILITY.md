...
Review `5260636868` found that the witness could still false-GREEN a repair that preserves three correct foundation branches while globally suppressing optional evidence. For example, adding `maxItems: 3` directly on `properties.evidence`, or adding a fourth `allOf` member such as `{ "maxItems": 3 }`, leaves the three `(1, 1)` foundation observations intact but prevents any `runtime_behavior`, `static_capability`, `network_attempt`, or `tool_failure` record from coexisting with the required foundation set. Test-only `ac480b481589c5251f41710cbb88d2da49c0614c` therefore constrains this repair surface to the existing evidence-array `type`, `minItems`, and `items` plus exactly one three-member `allOf`, and adds hostile controls for both a parent `maxItems: 3` sibling and a fourth unrelated `allOf` predicate. This does not change production Rust or the checked-in public schema; it prevents a cardinality repair from satisfying the owner witness by globally capping optional evidence.
...
After the current witness executes and fails because `properties.evidence.allOf` is absent, the minimum candidate GREEN is three direct subschemas. Each counted `allOf` member is itself narrow and unconditional: it contains only a narrow `contains` selector plus adjacent `minContains: 1` and `maxContains: 1`. The `contains` selector explicitly requires only `evidence_kind`, constrains only that property to the one expected foundation kind, and adds no unrelated predicate. The `evidence` array itself retains its existing `type`, `minItems`, and `items` contract plus only those three new `allOf` members; the cardinality repair must not introduce a parent `maxItems`, a fourth unrelated `allOf` branch, or another array-level restriction that prevents optional non-foundation evidence from coexisting with the required foundation records. The existing `items` schema remains the authority for evidence-record shape.

The repair must not singleton-constrain `static_capability`, `runtime_behavior`, `network_attempt`, or `tool_failure`. It must not use `minItems: 3`, fixed positions, `uniqueItems`, `producer_id`, schema-version rewriting, undocumented keywords, unrelated predicates inside a foundation `contains`, outer sibling assertions/applicators on a counted occurrence branch, parent-level cardinality caps, extra unrelated `allOf` members, or constraints hidden in dead/optional branches.

A different Draft 2020-12 construction is acceptable only if its test proves that the occurrence constraint is unconditionally effective for the evidence array and does not globally suppress non-foundation evidence, rather than merely proving keyword presence somewhere in the schema document.
