...
Draft #127 exact `e9ad58ceaec006b11da63772c22447b3a424da23` already separates the generated `qsr-net-*` correlation from the canonical backend network ID used for container creation. A later boundary still loses that authority.

`RootlessPodmanAdapter::launch_at()` obtains `network_id` from `acquire_network_id()` and replaces the container `--network` selector with that value. After the exact acquired container starts, `verify_effective_isolation()` receives the launch plan and container ID, but not the admitted network ID. Its P0 network-state check therefore executes `podman network inspect --format json <plan.network_name()>` again.
...
This is distinct from the create→first-inspect race already owned by parent #127. Fixing creation-bound admission does not by itself prevent a later security check from falling back to mutable correlation.

## Exact code and RED

Parent code authority: `src/infrastructure/podman.rs@e9ad58ceaec006b11da63772c22447b3a424da23`.

The relevant authority flow is:

1. `acquire_network_id(&plan)` returns a canonical backend ID;
2. `bind_network_selector(&mut create_args, &network_id)` binds container creation to it;
3. the exact acquired container is started;
4. `verify_effective_isolation(&plan, ..., &container_id)` runs without `network_id`;
5. the network P0 check resolves `plan.network_name()` again.
...
The witness requires post-start P0 network inspection to use the admitted immutable ID and forbids public-name re-resolution at that security boundary. It intentionally does not change production Rust/API/schema.

## Security invariant

Once immutable backend identity is admitted, all later network security or destructive authority must be carried by that ID or an equivalent private capability derived from it. The public correlation may remain in consumer-visible lease/audit data, but it must not be promoted back into:
...
Do not repair this by adding another expected-name predicate. Carry the admitted immutable network authority through the private launch lifecycle and into effective isolation verification. If post-start network state is inspected, address the admitted full ID. Effective container attachment must also match that same ID before readiness. Cleanup/termination remain exact-ID and non-force under their existing owner REDs.

If parent #127 replaces CLI name-based acquisition with a creation-bound backend API, adapt this RED to the new private capability without preserving obsolete call shape. The invariant is continuity of immutable authority from creation/admission through effective verification and lifecycle cleanup.

## Sequencing

#142 is dependent on #127 and must remain Draft while parent exact `e9ad58c... / 35598340133` is still unexecuted. Parent create→first-inspect causality is classified first. Then ordinary/non-force adoption may adapt this RED to the chosen creation-bound mechanism. No predecessor result is promoted to the child exact.
...
**Problem.** The adapter admits one immutable network ID but later drops it from the effective-verification function boundary and re-resolves a public correlation for P0 state.

**Constraint.** Preserve the public lease correlation, current DDD ownership, fail-closed P0 isolation, exact-container identity, non-force cleanup, and parent #127 causal ordering.
...
**Selected direction.** Preserve admitted immutable network authority as a private capability through effective isolation verification and later lifecycle operations.

**Effect.** A network object that merely wins the public name after admission cannot supply evidence for the exact container's isolation boundary.
