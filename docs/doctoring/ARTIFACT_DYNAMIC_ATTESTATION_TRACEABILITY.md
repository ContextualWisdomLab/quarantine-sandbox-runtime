...
- An unavailable dynamic worker remains `Inconclusive` with `dynamic_execution_performed=false`; the same state must not validate as `Completed`.
...
Selected direction after causal RED. Static-only receipts require no execution; unavailable dynamic receipts remain incomplete; completed dynamic receipts may represent actual execution only when attributable isolated-worker evidence exists. The selected wire schema must encode the same cross-field rule rather than validating each field in isolation.
