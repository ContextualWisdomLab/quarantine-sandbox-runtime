...
- Fresh review `5130709433` found that repair incomplete: seven P0 controls already represented by `VerifiedIsolationState` were not required to be positively verified before a worker receipt could be accepted.
- Current semantic RED is `tests/artifact_analysis_worker_required_isolation_controls_red.rs`, introduced at `32dbd922a431f8e8ced830c8c40e54259cbb1cdf`. It requires fail-closed handling when `rootless`, `read_only_root_filesystem`, `all_capabilities_dropped`, `no_new_privileges`, `isolated_user_namespace`, `seccomp_enforced`, or `lsm_enforced` is `Unavailable`. `loopback_only_publication=NotApplicable` remains intentional because analyzer workers are not application-service publication endpoints.

Transient queued/running workflow identifiers are deliberately kept in PR/Issue operational metadata rather than this versioned record. Historical CI is recorded here only after an unchanged semantic head actually executes the relevant gate.
...
The current RED exists because `c7a05a...` enforced only a subset of those canonical Core controls. The next minimum production repair, after the RED executes for that intended cause, is to extend `SandboxWorkerIsolationEvidence::boundary_violation()` to require the seven missing statuses to be `Verified`. Do not duplicate those statuses in `artifact_analysis` or add service-only loopback publication as a worker requirement.
