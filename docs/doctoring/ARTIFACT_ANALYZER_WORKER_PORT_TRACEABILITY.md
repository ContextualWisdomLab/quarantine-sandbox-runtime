...
- Native CI `34057398674` belongs to predecessor `f863a897...`; its five jobs were still pre-checkout queued with no runner assignment when the isolation-policy binding RED moved the branch. Predecessor checks do not transfer.
...
After causal RED execution, the smallest GREEN is the backend-neutral port/value-object implementation and validation required by the test, including exact request/receipt isolation-policy binding and requested/applied budget binding. It must not wire arbitrary external analyzers back into the controller process and must not add Podman-specific types to `artifact_analysis`.
