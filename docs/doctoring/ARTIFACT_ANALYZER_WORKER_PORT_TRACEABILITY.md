...
Candidate `4bc77fa89050f313f42ffccd0258bc107aeb0d17` added the first backend-neutral implementation. Exact-head CodeRabbit review then found two valid blocking defects against ADR-0009:
...
Current repair RED is test-only `f76299c27cbeef57397d166c9b9e476c0933c749`, adding `tests/artifact_analysis_worker_core_boundary_red.rs`. It requires reusable worker resource/isolation/lifecycle values to be Core-owned as `SandboxWorkerBudget`, `SandboxWorkerIsolationEvidence`, `SandboxWorkerTerminationEvidence`, and `SandboxWorkerTerminationState`; it rejects duplicate `AnalyzerWorkerBudget` / `AnalyzerWorkerIsolationEvidence` definitions in the Supporting context; and it requires receipt validation to fail when termination is non-terminal or belongs to a different worker identity. Native CI `34072872833` is the current execution authority for this repair RED. At the documenting sweep its verify `101593358101`, branch coverage `101593358269`, positive-LSM `101593358277`, hosted negative rootless/AppArmor `101593358325`, and coverage `101593358338` were pre-checkout queued. Production repair must wait for causal execution of this exact repair RED or a later docs-equivalent descendant preserving the identical test delta.
...
The artifact-analysis request must still bind immutable analyzer implementation identity, exact artifact identity, policy identity, expected immutable isolation-policy SHA-256, and the Core-owned worker budget. A worker receipt must bind those request inputs to runtime-owned worker/backend/policy/budget/lifecycle evidence and normalized analyzer output. Controller-side validation must reject contradictions, malformed identity, denied-capability violations, a non-terminal worker, termination evidence for another worker, cleanup uncertainty, and malformed per-item outcomes before downstream evidence ingestion.
...
The first missing-port RED has executed. The current repair RED intentionally demands Core-owned `SandboxWorker*` isolation/lifecycle values that the reviewed candidate does not provide. The smallest causal repair, after this RED executes for its intended cause, is to move reusable budget/isolation/termination/cleanup semantics into `sandbox_execution`, change the analyzer request/receipt to compose those Core values, remove the duplicate Supporting-context isolation structs, and reject non-terminal or wrong-worker termination evidence.
