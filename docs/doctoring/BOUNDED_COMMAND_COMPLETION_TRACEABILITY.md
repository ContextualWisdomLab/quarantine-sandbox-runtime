...
One-shot run `34729014658` validated the repair after two source-fixer definition defects had themselves been repaired. It preserved the already-executed branch RED, applied the minimum source/doc change, completed the focused classifier and supervision tests, full locked workspace/all-target tests, Clippy with warnings denied, rustdoc with warnings denied, rustfmt, and `git diff --check`, then published executable source `5dfb8bd156202917f60a5130124741b139295702` and removed the purpose-complete workflow in the same commit. Native CI `34729073953` on that bot-authored executable commit was classified `action_required`; no exact-head GREEN transfers from the one-shot run. This documentation-only descendant exists solely to rematerialize native CI without changing executable semantics.
