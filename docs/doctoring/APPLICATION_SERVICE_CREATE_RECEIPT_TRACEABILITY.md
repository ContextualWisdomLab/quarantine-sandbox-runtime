...
The purpose-bound repair run establishes local whole-workspace GREEN for the causal source change, but its bot-authored descendant does not substitute for the repository's native exact-head CI/security evidence. A connector-authored documentation descendant must reacquire the normal verify, production coverage, branch coverage, and hosted-negative lanes on one unchanged head; dedicated positive effective-LSM remains separately required.
