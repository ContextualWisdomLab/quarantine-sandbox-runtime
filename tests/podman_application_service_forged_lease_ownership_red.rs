//! Regression: caller-deserialized lease evidence must never recreate destructive cleanup authority.
//!
//! Application-service lease fields are public evidence/correlation data. A caller-controlled
//! serialized lease can name arbitrary resources, so termination must require runtime-private
//! cleanup authority that is present only on a lease created by this process.
...
fn temporary_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "qsr-application-service-forged-lease-integration-{name}-{}-{nanos}-{unique_id}",
        std::process::id()
    ))
}
...
fn forged_lease() -> ApplicationServiceLease {
    ...
    serde_json::from_str(&json).expect("current public lease wire shape must deserialize")
}
...
fn deserialized_lease_cannot_select_destructive_backend_resources() {
    ...
    assert_eq!(
        ...
        "safe\n",
        "forged lease identifiers must never select foreign resources for destruction"
    );

    let calls = fs::read_to_string(&log).unwrap_or_default();
    assert!(
        calls.trim().is_empty(),
        "termination must reject forged authority before invoking Podman; calls were:\n{calls}"
    );
