//! Regression coverage for bounded Podman CLI execution.
...
fn write_hanging_podman() -> (PathBuf, PathBuf) {
    let program = temporary_path("hanging-podman");
    let pid_file = temporary_path("hanging-podman-pid");
    let script = format!(
        "#!/bin/sh\nset -eu\nif [ \"${{1:-}}\" = info ]; then\n  printf '%s\\n' \"$$\" > '{}'\n  exec sleep 60\nfi\nexit 91\n",
        pid_file.display()
    );
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "bounded_podman_command_policy_v1".to_owned(),
...
fn request() -> ApplicationServiceRequest {
    ApplicationServiceRequest {
        ...
        request_id: "bounded_podman_command_request".to_owned(),
...
fn hanging_backend_command_is_killed_reaped_and_reported_as_timeout() {
    ...
    assert!(
        ...
        "a hung backend command must not consume an unbounded wall-clock budget"
    );
    ...
    assert!(
        !proc_path.exists(),
        "timed-out Podman process must be killed and reaped"
    );
