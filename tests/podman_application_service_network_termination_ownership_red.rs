//! Regression: explicit service termination must not delete foreign network members.
...
fn temporary_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "qsr-network-termination-ownership-red-{name}-{}-{nanos}-{unique_id}",
        std::process::id()
    ))
}
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "network_termination_ownership_red_v1".to_owned(),
...
fn request() -> ApplicationServiceRequest {
    ApplicationServiceRequest {
        ...
        request_id: "network_termination_ownership_red".to_owned(),
...
fn write_fake_podman(foreign_marker: &PathBuf, ready_port: u16) -> (PathBuf, PathBuf) {
    ...
    let network = format!(
        r#"[{{"id":"{}","internal":true,"dns_enabled":false}}]"#,
        "a".repeat(64)
    );
    let script = format!(
...
#[test]
fn explicit_termination_never_force_removes_foreign_network_members() {
    ...
    let (program, log) = write_fake_podman(&foreign_marker, ready_port);
    let adapter = RootlessPodmanAdapter::new(program.clone());

    let lease = adapter
        .launch_at(&request(), &policy(), 1_780_000_300)
        .expect("the service must launch before the foreign member appears");
    fs::write(&foreign_marker, "safe\n").expect("foreign marker must be writable");

    let result = adapter.terminate_at(&lease, 1_780_000_310);
    assert_eq!(
        result,
        Err(ApplicationServiceError::CleanupFailed),
        "an in-use sandbox network must surface cleanup uncertainty during explicit termination"
    );
    assert_eq!(
        fs::read_to_string(&foreign_marker).expect("foreign marker must remain readable"),
        "safe\n",
        "explicit termination must never delete a foreign container through network-level force removal"
    );
    ...
    assert!(
        ...
        "the explicit network cleanup path must be exercised; calls were:\n{calls}"
    );
    assert!(
        !calls
            .lines()
            .any(|line| line.starts_with("network rm --force ")),
        "explicit termination must not delegate foreign-container deletion to `podman network rm --force`; calls were:\n{calls}"
    );
