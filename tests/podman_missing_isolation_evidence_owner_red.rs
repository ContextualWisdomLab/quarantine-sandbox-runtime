//! Owner-path RED coverage for required Podman isolation fields.
//!
//! These fake-Podman fixtures verify only infrastructure evidence admission.
//! They do not substitute for real rootless or positive-LSM release evidence.
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "missing_isolation_evidence_owner_v1".to_owned(),
...
fn request() -> ApplicationServiceRequest {
    ApplicationServiceRequest {
        ...
        request_id: "missing_isolation_evidence_owner_request".to_owned(),
...
fn assert_rejected_before_port(
    ...
    assert!(
        ...
        "missing P0 evidence must fail before port/readiness: {calls}"
    );
}
...
fn explicit_secure_values_remain_distinct_from_missing_evidence() {
    let (result, calls) = launch(Fixture::default());
    assert_eq!(result, Err(ApplicationServiceError::ReadinessTimeout));
    assert!(
        calls.lines().any(|line| line.starts_with("port ")),
        "explicit secure values should reach the independent readiness boundary: {calls}"
    );
}
