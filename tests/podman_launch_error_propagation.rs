//! Fail-closed launch error propagation at the real Podman process boundary.
...
fn temporary_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "quarantine-sandbox-runtime-launch-error-{name}-{}-{nanos}-{unique_id}",
        std::process::id()
    ))
}
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "launch_error_policy_v1".to_owned(),
...
fn request() -> ApplicationServiceRequest {
    ApplicationServiceRequest {
        ...
        request_id: "launch_error_request".to_owned(),
...
#[test]
fn launch_rejects_invalid_request_before_invoking_the_backend() {
    let missing_program = temporary_path("missing-podman");
    let adapter = RootlessPodmanAdapter::new(missing_program);
    let mut invalid_request = request();
    invalid_request.container_port = 0;

    assert_eq!(
        adapter.launch_at(&invalid_request, &policy(), 1_780_000_000),
        Err(ApplicationServiceError::InvalidContainerPort)
    );
}

#[test]
fn invalid_port_cleanup_failure_overrides_mapping_error_after_all_cleanup_attempts() {
