//! Owner-path RED for adopting bounded backend spawn-failure evidence onto the network lineage.
//!
//! Historical #124 proved that a missing backend executable must remain distinguishable from
//! post-spawn wait/capture failures. The canonical network successor must preserve that contract
//! without importing the older branch wholesale or weakening its network ownership work.
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "backend_spawn_succession_v1".to_owned(),
...
fn request() -> ApplicationServiceRequest {
    ApplicationServiceRequest {
        ...
        request_id: "missing_backend_spawn_succession".to_owned(),
...
fn owned_empty_directory() -> PathBuf {
    for attempt in 0_u32..1_000 {
        let directory = std::env::temp_dir().join(format!(
            "quarantine-sandbox-runtime-spawn-succession-red-{}-{attempt}",
            std::process::id()
        ));
