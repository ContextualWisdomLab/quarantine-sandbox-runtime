//! RED regression for authoritative `podman wait` evidence.
//!
//! A nonzero administrative `podman wait` status means the runtime failed to
//! obtain trustworthy completion evidence. Its stdout must not be promoted to a
//! workload exit code, and the command sandbox still has to be removed.
...
fn temporary_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "quarantine-sandbox-runtime-command-wait-status-red-{name}-{}-{nanos}-{unique_id}",
        std::process::id()
    ))
}
...
fn policy() -> IsolationPolicy {
    IsolationPolicy {
        policy_id: "command_wait_status_red_policy_v1".to_owned(),
...
#[test]
fn failed_wait_command_is_not_promoted_to_workload_exit_status() {
